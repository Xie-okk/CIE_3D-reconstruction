...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.4\columnwidth]{fig_coordinate.png}}
...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.6\columnwidth]{fig_radar_los.png}}
...
\begin{table}[htbp]
	...
	\centering
	\renewcommand{\arraystretch}{1.2} % 增加行高，使垂直居中更美观
...
Based on the ISAR image envelopes, the dimensions of the reference triaxial ellipsoid are pre-estimated as $a=20$~m, $b=10$~m, and $c=10$~m. With this geometric baseline, the estimation process is initiated via a wide-bound blind search with all attitude parameters initialized to zero. The optimization aims to minimize the proposed envelope-based cost function to lock onto the kinematic state, with the results of a representative initial run summarized in Table~\ref{tab_estimation_results}.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		\textbf{Symbol} & \textbf{Bounds} & \textbf{Initial} & \textbf{Truth} & \textbf{Estimated} & \textbf{Error} \\ \hline
		$T$ (h) & $[8.0, 12.0]$ & 8.00 & 9.00 & 8.99 & 0.01 \\ \hline
		$\lambda$ ($^{\circ}$) & $[0, 360]$ & 0.0 & 23.3 & 17.2 & 6.1 \\ \hline
		$\beta$ ($^{\circ}$) & $[-90, 90]$ & 0.0 & 45.0 & 55.1 & 10.1 \\ \hline
		$\phi_0$ ($^{\circ}$) & $[0, 180]$ & 0.0 & 60.0 & 67.3 & 7.3 \\ \hline
	\end{tabular}
\end{table}

As shown in Table~\ref{tab_estimation_results}, the algorithm accurately captures the spin period $T$, even under severe noise. However, the pole latitude $\beta$ shows a deviation of $10.1^{\circ}$. This is a typical "modeling error" where the optimization compensates for the morphological mismatch between the triaxial ellipsoid model and the asymmetric asteroid by slightly tilting the observation perspective.

To evaluate estimation stability, multiple independent blind searches were conducted within a narrowed constraint interval for $T$ (Table~\ref{tab_stability_comparison}). While $T$ and $\phi_0$ remain stable, the pole directions ($\lambda, \beta$) exhibit significant variance. This ill-posedness is driven by the 59.2-hour observation gap, where micro-errors in $T$ accumulate into phase drifts, forcing the algorithm into various compensatory local minima.

\begin{table}[htbp]
	\caption{Comparison of Estimation Stability: Blind Search vs. Prior Spin Period}
	\label{tab_stability_comparison}
	\centering
	\renewcommand{\arraystretch}{1.2}
	\setlength{\tabcolsep}{8pt} % 适当增加列间距，让表格看起来更舒展
	\begin{tabular}{|c|c|c|c|}
		\hline
		\textbf{Symbol} & \textbf{Truth} & \textbf{Blind Search (Mean $\pm$ Std.)} & \textbf{Fixed Period} \\ \hline
		$T$ (h) & 9.00 & $8.99 \pm 0.02$ & \textbf{9.00} \\ \hline
		$\lambda$ ($^{\circ}$) & 23.3 & $19.9 \pm 7.1$ & \textbf{20.6} \\ \hline
		$\beta$ ($^{\circ}$) & 45.0 & $58.1 \pm 8.2$ & \textbf{47.0} \\ \hline
		$\phi_0$ ($^{\circ}$) & 60.0 & $66.7 \pm 0.8$ & \textbf{60.3} \\ \hline
	\end{tabular}
\end{table}

In practical missions, the spin period is often known from optical lightcurves. By introducing this prior ($T = 9.00$~h), the phase drift is eliminated. As shown in Table~\ref{tab_stability_comparison}, this constraint forces the algorithm out of the compensatory minima, reducing the $\beta$ error to only $2.0^{\circ}$. This configuration provides a robust initialization for the subsequent 3D shape deformation.

To qualitatively evaluate the final kinematic estimation, the simulated 2D ISAR projections based on the prior-constrained parameters are compared with the ground truth in Fig.~\ref{fig_disp_full_width}. The rotation rhythm and orientation of the simulated envelopes are highly consistent with the observations, validating the estimated kinematic parameters. However, the noticeable "structural gap" between the ellipsoid and the actual shape provides the necessary residual information for the subsequent shape deformation.

\begin{figure*}[htbp]
	\centering
	\includegraphics[width=\textwidth]{imgResult_rotation2026-04-10-11-49.png}
	\caption{Comparison of observed and simulated ISAR images (36 frames in total). Rows 1 and 3 display the ground truth ISAR projections for Epoch 1 (Frames 1--9) and Epoch 2 (Frames 10--18), respectively. Rows 2 and 4 show the corresponding simulations of the initial ellipsoid model using the prior-constrained rotation parameters.}
